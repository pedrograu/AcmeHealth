...
<div class="table-responsive">
	<display:table name="offers" id="row" requestURI="${requestURI}"
		...
		<security:authorize access="isAnonymous()">
			<display:column>
				<a href="offer/details.do?offerId=${row.id}"><spring:message
						code="offer.details" /></a>
			</display:column>
		</security:authorize>

		<security:authorize access="hasRole('PATIENT')">
			<display:column>
				<a href="offer/patient/detail.do?offerId=${row.id}"><spring:message
						code="offer.details" /></a>
			</display:column>
		</security:authorize>
		...
		<security:authorize access="hasRole('ADMINISTRATOR')">
			<display:column>
				<a href="offer/administrator/detail.do?offerId=${row.id}"><spring:message
						code="offer.details" /></a>
			</display:column>
			<jstl:if test="${own==true}">
				<display:column>
					<a href="offer/administrator/delete.do?offerId=${row.id}"><spring:message
							code="offer.delete" /></a>
				</display:column>
			</jstl:if>
		</security:authorize>
		...
		<security:authorize access="hasRole('SPECIALIST')">
			<display:column>
				<a href="offer/specialist/detail.do?offerId=${row.id}"><spring:message
						code="offer.details" /></a>
			</display:column>
		</security:authorize>
		...
		<security:authorize access="hasRole('PATIENT')">
			<jstl:if test="${notFinish==true}">
				<display:column>
					<a href="offer/patient/hire.do?offerId=${row.id}"><spring:message
							code="offer.hire" /></a>
				</display:column>
			</jstl:if>
			...
			<jstl:if test="${hire==true}">
				<display:column>
					<a href="appointment/patient/calendar2.do?offerId=${row.id}"><spring:message
							code="offer.selectApointment" /></a>
				</display:column>
			</jstl:if>
		</security:authorize>
